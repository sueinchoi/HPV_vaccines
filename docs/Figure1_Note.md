...
- A strict fixed 1:4 fine-matching alternative was prespecified as a sensitivity analysis (Supplementary Table on strict matching). Strict matching dropped 48 of 241 vaccinated cases (20%) and 16% of recurrence events while yielding essentially identical hazard-ratio estimates (lesion recurrence HR 0.69, 95% CI 0.36–1.33 vs. primary 0.76, 95% CI 0.43–1.33; HPV reinfection HR 0.94, 95% CI 0.79–1.12 vs. primary 0.97, 95% CI 0.83–1.14). Variable-ratio matching was preferred for the primary analysis because (i) it preserves all vaccinated cases for which any acceptable comparator exists—important given the limited number of vaccinated surgical patients and the small absolute event count (13 lesion recurrences in the vaccinated arm), and (ii) cluster-robust standard errors based on the matched-set identifier appropriately accommodate the unequal cluster sizes.

### e. Power and statistical caveats
- The detectable hazard ratio at 80% power and α = 0.05 is approximately 1.95 for lesion recurrence and 1.24 for HPV reinfection, indicating that the primary analysis is well powered to detect a doubling of risk for HPV reinfection but only large protective effects for lesion recurrence.
